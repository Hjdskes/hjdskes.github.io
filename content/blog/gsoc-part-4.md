...
```
    def do_add(self, widget):
        """Adds the given widget to the map.
...
width. Since the MouseMap is static with regards to geometry, we simply return
the height (or width) or the SVG, including the border width and any children
that are drawn above or below (or left and right) of the SVG.
...
```python
    def do_get_preferred_height(self):
        ...
        SVG's height, including the border width."""
        # TODO: account for children and spacing
    ...
    def do_get_preferred_width(self):
        ...
        not to get) we cannot be certain that our wishes are granted and hence
        we must implement this method as well. In any case, we just return the
        SVG's width, including the border width."""
        # TODO: account for children and spacing
        width = self._handle.props.width + 2 * self.props.border_width
        return (width, width)
```
...
```python
    def do_size_allocate(self, allocation):
        ...
        """
        # TODO: some SVGs also have leaders pointing to the left.
        ...
        for child in self._children:
            try:
                ...
                print("Warning: child does not have `id` attribute, skipping", file=sys.stderr)
                continue
            if child.get_visible():
                ...
                if not ok:
                    ...
                    continue
                child_allocation.x = svg_pos.x + svg_dim.width + self.spacing
                child_allocation.y = svg_pos.y - 0.5 * child_allocation.height
...
It's rather self explanatory: to draw the SVG in the center, we retrieve the
center coordinates of the container and translate Cairo's drawing context to
these coordinates minus half of the SVG's width and height. We then ask the SVG
if it has the required layers and if so, we draw only those. Otherwise, we draw
the entire SVG. Finally, we propagate the draw signal to all children so they
draw themselves on top of the SVG.
...
```python
    def do_draw(self, cr):
        ...
        def _get_center_coordinates(self):
            ...
            x = allocation.width / 2 - 2 * self.props.border_width;
            y = allocation.height / 2 - 2 * self.props.border_width;
            return x, y
        x, y = _get_center_coordinates(self)
        cr.translate(x - self._handle.props.width / 2, y - self._handle.props.height / 2)
...
when the mouse cursor enters and leaves the widget's window. The code still
needs polishing (for instance, it does not yet work in case the SVG isn't
found nor any error checking), but here it is:
...
```python
    def _on_enter(self, widget, event):
        ...
        ok, file_data = GLib.file_get_contents(self._device.svg_path)
        if not ok:
            return
...
</svg>""" % (svg_width, svg_height, svg_id, escaped_file_data)
        stream = Gio.MemoryInputStream.new_from_data(svg_data.encode('UTF-8'), None)
        self._handle = Rsvg.Handle.new_from_stream_sync(stream, None, Rsvg.HandleFlags.FLAGS_NONE, None)
        # TODO: make this queue_draw_area(x, y, width, height) to only redraw the SVG
        self.queue_draw()
...
All that work has resulted in the following GIF:

![Interactive MouseMap widget](/img/blog/gsoc-part-4/mousemap-interactive.gif)
